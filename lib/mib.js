...
var MIB = function () {
    return ({
        ...
        Import: function (FileName) {
            ...
            this.ParseModule(fname, fs.readFileSync(FileName).toString());
        },
                            ...
                            while (Symbols[tmp] != ';') {
                                if (Symbols[tmp] == 'FROM') {
                                    ...
                                    if (!this.Modules[ImportModule]) {
                                        ...
                                        console.log(ModuleName+': Attempting to load '+ImportModule);
                                        this.CompileModule(ImportModule);
                                        if (!this.Modules[ImportModule]) 
                                            console.log(ModuleName + ': Can not find ' + ImportModule + '!!!!!!!!!!!!!!!!!!!!!');
        ...
        LoadMIBpath: function(mibPath) {
            fs.readdirSync(mibPath).forEach(file => {
                this.Import(path.join(mibPath,file));
              })
              this.Serialize();
        },
        LoadMIBs: function () {
